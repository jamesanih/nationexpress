<?php
	...
	if($_POST['email']!='')	{
		...
		$alogin_id = GenaloginID();
		$ulogin_id = GenuloginID();
		$ilogin_id = GeniloginID();
		...
		if($row_admin > 0 && $val_admin['status'] == "Enabled") {
			...
			$query_login = mysqli_query($connect,$connect, "SELECT * FROM `admin_login_details` WHERE `admin_id`='{$val_admin['admin_id']}' AND `email`='{$val_admin['email']}' order by id DESC LIMIT 1");
