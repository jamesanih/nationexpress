<?php
	...
	if($myno !="") {
		...
		$sql_invoice = mysqli_query($connect, "SELECT * FROM `delivery_details` WHERE `booking_no`='$myno' ORDER BY id DESC LIMIT 1");
		...
		$sql_register= mysqli_query($connect, "SELECT * FROM `register` WHERE `account_id`='$myid_exists' ORDER BY id DESC LIMIT 1");
		$row_register = mysqli_num_rows($sql_register);
		$val_register1 = mysqli_fetch_assoc($sql_register);
		...
		$query_check4 = mysqli_query($connect, "SELECT * FROM `settings_delivery_type` WHERE `status` = 'Active' ORDER BY id DESC");
		$row_check4 = mysqli_num_rows($sql_check4);
		$val4 = mysqli_fetch_assoc($query_check4);
	...
	if(isset($_POST['submit_button'])){
		...
		elseif($myaccount_id_email){
			...
			if($qty=='' || $acct_id=='' || $weight=='' || $payment_method=='' || $delivery_cost == '' || $description == '') {
				...
				$sql_pk1 = mysqli_query($connect, "SELECT * FROM `pickup_details` WHERE `booking_no` = '$booking_no' AND `account_id`='$myaccount_id_email' ORDER BY ID DESC LIMIT 1");
				$row_pk1 = mysqli_num_rows($sql_pk1);
				$val_pk1 = mysqli_fetch_assoc($sql_pk1);

				$pk_city = $val_pk1['city'];

				//Check if parcel records exist

				$sql_parcel = mysqli_query($connect, "SELECT * FROM `parcel_details` WHERE `booking_no` = '$booking_no' AND `account_id`='$myaccount_id_email' ORDER BY ID DESC LIMIT 1");
				$row_parcel = mysqli_num_rows($sql_parcel);
				$val_parcel = mysqli_fetch_assoc($sql_parcel);

				$parcel_id = $val_parcel['account_id'];

				//Check if payment records exist

				$sql_pay = mysqli_query($connect, "SELECT * FROM `payment_details` WHERE `booking_no` = '$booking_no' AND `account_id`='$myaccount_id_email' ORDER BY ID DESC LIMIT 1");
				$row_pay = mysqli_num_rows($sql_pay);
				$val_pay = mysqli_fetch_assoc($sql_pay);
